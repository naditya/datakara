...
class Suku extends CI_Controller {
	...
	function render_page_topadmin_add($content, $data = NULL){
        ...
        $this->load->view('v_tambahsuku', $data);
    }
	...
	function render_page_topadmin_edit($content, $data = NULL){
        ...
        $this->load->view('v_editsuku', $data);
    }
	...
	public function index() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$table='suku';
			$data['suku'] = $this->model_crud->view_data($table)->result();
			$this->render_page_topadmin('v_suku', $data);
		}
	...
	public function form() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$this->render_page_topadmin_add('v_tambahsuku');
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	public function addsuku(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$data=array(
				'id_suku'=>'',
				'nama_suku'=>$this->input->post('val-username')
			);
	...
	public function dataform($uptome) {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			//echo "<br> $id";
			$where = array('id_suku' => $uptome);
			$table = 'suku';
			$data['suku'] = $this->model_crud->getbyid($where,$table)->result();
			$this->render_page_topadmin_edit('v_editsuku', $data);
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	function editdata(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$namasuku = $this->input->post('val-username');
