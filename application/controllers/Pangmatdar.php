...
class Pangmatdar extends CI_Controller {
	...
	function render_page_topadmin($content, $data = NULL){
        ...
        $this->load->view('v_pangmatdar', $data);
    }
	...
	function render_page_topadmin_add($content, $data = NULL){
        ...
        $this->load->view('v_tambahpangmatdar', $data);
    }
	...
	function render_page_topadmin_edit($content, $data = NULL){
        ...
        $this->load->view('v_editpangmatdar', $data);
    }
	...
	public function index() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$table='pangmatdar';
			$data['pangmatdar'] = $this->model_crud->view_data($table)->result();
			$this->render_page_topadmin('v_pangmatdar', $data);
		}
	...
	public function form() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$this->render_page_topadmin_add('v_tambahpangmatdar');
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	public function addpangmatdar(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$data=array(
				'id_pangmatdar'=>'',
				'nama_pangmatdar'=>$this->input->post('val-username')
			);
	...
	public function dataform($uptome) {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			//echo "<br> $id";
			$where = array('id_pangmatdar' => $uptome);
			$table = 'pangmatdar';
			$data['pangmatdar'] = $this->model_crud->getbyid($where,$table)->result();
			$this->render_page_topadmin_edit('v_editpangmatdar', $data);
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	function editdata(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$namapangmatdar = $this->input->post('val-username');
			
		 
			$data = array(
				'nama_pangmatdar' => $namapangmatdar
			);
