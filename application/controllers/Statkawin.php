...
class Statkawin extends CI_Controller {
	...
	function render_page_topadmin($content, $data = NULL){
        ...
        $this->load->view('v_statkawin', $data);
    }
	...
	function render_page_topadmin_add($content, $data = NULL){
        ...
        $this->load->view('v_tambahstatkawin', $data);
    }
	...
	function render_page_topadmin_edit($content, $data = NULL){
        ...
        $this->load->view('v_editstatkawin', $data);
    }
	...
	public function index() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$table='statkawin';
			$data['statkawin'] = $this->model_crud->view_data($table)->result();
			$this->render_page_topadmin('v_statkawin', $data);
		}
	...
	public function form() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$this->render_page_topadmin_add('v_tambahstatkawin');
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	public function addstatkawin(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$data=array(
				'id_statkawin'=>'',
				'nama_statkawin'=>$this->input->post('val-username')
			);
	...
	public function dataform($uptome) {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			//echo "<br> $id";
			$where = array('id_statkawin' => $uptome);
			$table = 'statkawin';
			$data['statkawin'] = $this->model_crud->getbyid($where,$table)->result();
			$this->render_page_topadmin_edit('v_editstatkawin', $data);
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	function editdata(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$namastatkawin = $this->input->post('val-username');
			
		 
			$data = array(
				'nama_statkawin' => $namastatkawin
			);
