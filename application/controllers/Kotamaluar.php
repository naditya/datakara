...
class Kotamaluar extends CI_Controller {
	...
	function render_page_topadmin($content, $data = NULL){
        ...
        $this->load->view('v_kotamaluar', $data);
    }
	...
	function render_page_topadmin_add($content, $data = NULL){
        ...
        $this->load->view('v_tambahkotamaluar', $data);
    }
	...
	function render_page_topadmin_edit($content, $data = NULL){
        ...
        $this->load->view('v_editkotamaluar', $data);
    }
	...
	public function index() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$table='kotamaluar';
			$data['kotamaluar'] = $this->model_crud->view_data($table)->result();
			$this->render_page_topadmin('v_kotamaluar', $data);
		}
	...
	public function form() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$this->render_page_topadmin_add('v_tambahkotamaluar');
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	public function addkotamaluar(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$data=array(
				'id_kotamaluar'=>'',
				'nama_kotamaluar'=>$this->input->post('val-username')
			);
	...
	public function dataform($uptome) {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			//echo "<br> $id";
			$where = array('id_kotamaluar' => $uptome);
			$table = 'kotamaluar';
			$data['kotamaluar'] = $this->model_crud->getbyid($where,$table)->result();
			$this->render_page_topadmin_edit('v_editkotamaluar', $data);
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	function editdata(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$namakotamaluar = $this->input->post('val-username');
			
		 
			$data = array(
				'nama_kotamaluar' => $namakotamaluar
			);
