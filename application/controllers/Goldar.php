...
class Goldar extends CI_Controller {
	...
	function render_page_topadmin_add($content, $data = NULL){
        ...
        $this->load->view('v_tambahgoldar', $data);
    }
	...
	function render_page_topadmin_edit($content, $data = NULL){
        ...
        $this->load->view('v_editgoldar', $data);
    }
	...
	public function index() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$table='goldar';
			$data['goldar'] = $this->model_crud->view_data($table)->result();
			$this->render_page_topadmin('v_goldar', $data);
		}
	...
	public function form() {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			$this->render_page_topadmin_add('v_tambahgoldar');
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	public function addgoldar(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$data=array(
				'id_goldar'=>'',
				'nama_goldar'=>$this->input->post('val-username')
			);
	...
	public function dataform($uptome) {
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			//echo "<br> $id";
			$where = array('id_goldar' => $uptome);
			$table = 'goldar';
			$data['goldar'] = $this->model_crud->getbyid($where,$table)->result();
			$this->render_page_topadmin_edit('v_editgoldar', $data);
		}
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "admin" ) {
			$this->render_page_admin('a_dashboard');
		}
	...
	function editdata(){
		...
		if ($this->session->userdata('status') == "login" && $this->session->userdata('role') == "topadmin" ) {
			...
			$namagoldar = $this->input->post('val-username');
			
		 
			$data = array(
				'nama_goldar' => $namagoldar
			);
